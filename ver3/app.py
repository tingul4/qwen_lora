...
class Pipeline:
    ...
    def run_vlm_inference(self, frame_data):
        ...
        return output_text, user_prompt_text

    def visualize_frame(self, frame_data):
        img_cv = cv2.cvtColor(np.array(frame_data['image']), cv2.COLOR_RGB2BGR)
        
        # 1. Identify objects to draw (Top risk pair only)
        target_obj_ids = set()
        top_collision = None
        
        if 'collisions' in frame_data['json_desc'] and frame_data['json_desc']['collisions']:
            # Sort by probability descending
            collisions = sorted(frame_data['json_desc']['collisions'], key=lambda x: x['prob'], reverse=True)
            top_collision = collisions[0]
            target_obj_ids.add(top_collision['obj1']['id'])
            target_obj_ids.add(top_collision['obj2']['id'])
        ...
        if 'all_objects' in frame_data['json_desc']:
            for obj in frame_data['json_desc']['all_objects']:
                ...
                if obj['id'] not in target_obj_ids:
                    continue
                ...
                if obj.get('is_stationary', False):
                    ...
                    status.append("change lanes")
                
                if status:
                    cv2.putText(img_cv, " ".join(status), (x1, y1-15), 
                               cv2.FONT_HERSHEY_SIMPLEX, 0.5, box_color, 2)

        # 3. Draw Collision Line (Only for top collision)
        if top_collision:
            c = top_collision
            o1 = c['obj1']
            o2 = c['obj2']
...
def analyze_video_stream(source, video_id, history):
...
with gr.Blocks(theme=my_theme) as demo:
    ...
    def update_video_list(source):
        videos = get_video_list(source)
        if not videos:
             return gr.Dropdown(choices=[], value=None)
        return gr.Dropdown(choices=videos, value=videos[0])
    
    source_dropdown.change(fn=update_video_list, inputs=source_dropdown, outputs=video_dropdown)
    ...
    demo.load(fn=update_video_list, inputs=source_dropdown, outputs=video_dropdown)
